      ...
      $(function (){
		$(".btn-changepass").click(function(){
			if($("#newpass").val() != $("#conpass").val()){
				$(".errormessage").text("Error, Password does not match!")
			}
			if($("#newpass").val().length < 7 || $("#conpass").val() < 7){

				if($("#newpass").val().length == 0 || $("#conpass").val() == 0){
					$(".errormessage").text("Error, Password fields are empty!")
				}else{
					$(".errormessage").text("Error, You got a weak Password!")
				}
			}
			if($("#newpass").val() == $("#conpass").val()){
				var passdata = {'cpassword':$("#newpass").val()}
				$.ajax({
					url: "Controller/changepassword.php", //filterFile(),
					dataType: "json",
				...
				$(".errormessage").text("Success, Password updated!")
